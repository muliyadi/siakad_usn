...
$data['group_wa']=$row->group_wa;
          ...
          //  $list=(object)$list;
            ...
            $this->template->load($this->view, 'prodi/absen/list_jadwal_kuliah', $datax);
    ...
    function aabsen()
    {
        ...
        if(!$cek)
        {
         ...
         redirect($this->input->post('link_kelas'),'refresh'); 
     ...
     function cpertemuan($kd_jadwal)
    {
      ...
      $jadwal=$this->Akademika_model->get_jadwal($kd_jadwal);
       ...
       if($matakuliahd)
       {
          ...
          $this->template->load($this->view, 'prodi/absen/fabsen', $data);
    }
    ...
    function labsen($kd_jadwal)
    {
        ...
        $key['kd_jadwal']=$kd_jadwal;
        $sql="SELECT  absensih.id_absen,absensih.aktif,absensih.durasi_absen,absensih.materi,absensih.kd_jadwal,absensih.tgl_pertemuan, absensih.pertemuan_ke,SUM(h) as h,SUM(i)as i,SUM(s) as s,SUM(a) as a from absensih left join absensid on absensih.id_absen=absensid.id_absen where absensih.kd_jadwal='".$kd_jadwal."' GROUP by absensih.kd_jadwal,absensih.id_absen";
        $data['list_absen']=$this->db->query($sql)->result(); 
        $data['kd_jadwal']=$kd_jadwal;
        $data['matakuliah']=$this->get_mtk($kd_jadwal);
        $this->template->load($this->view, 'prodi/absen/labsen', $data);
    }
     function detail_absensi($id_absensi)
    {
         $this->cek();
        $data['list_absensi']=$this->get_detail_absensi($id_absensi);
      $this->template->load($this->view, 'prodi/absen/detail_absensi', $data);
    }
  ...
  `mahasiswa`.`tempat_lahir`, `mahasiswa`.`tgl_lahir`, absensid.h,absensid.i,absensid.s,absensid.a
FROM
  `absensih` left JOIN absensid on absensih.id_absen=absensid.id_absen inner join
  `mahasiswa` ON `absensid`.`nim` = `mahasiswa`.`nim`  where absensih.id_absen='".$id_absensi."'  order by nm_mahasiswa asc";
    ...
    function fudo()
	{
        ...
        $data['listangkatan'] = $this->Akademika_model->get_all_angkatan();
		 ...
		 //$data['listmhs'] = $this->get_mhs_udo($homebase);
		  ...
		  //filder do
		    $this->template->load($this->view, 'prodi/do/form_filter_do', $data);
	...
	{
	    $sql="SELECT doh.no_do,tgl_do,kd_tahun_ajaran,count(*) as jumlah from doh,dod where doh.no_do=dod.no_do and kd_prodi='".$kd_prodi."' group by doh.no_do ";
        $listdo= $this->db->query($sql)->result();
       return ($listdo);
	}
...
function edit_usulan_judul($no_daftar)
{
    ...
    $usulan=$this->Prodi_model->get_row_selected('vdaftar_judul',$keydaftar);
    $data['aksi']='edit';
    ...
    $filter1['no_daftar']=$no_daftar;
    $filter1['pembimbing_ke']='1';
    
    
    $pembimbing1=$this->Prodi_model->get_row_selected('vpembimbing_skripsi',$filter1);
    
    $filter2['no_daftar']=$no_daftar;
    $filter2['pembimbing_ke']='2';
     $pembimbing2=$this->Prodi_model->get_row_selected('vpembimbing_skripsi',$filter2);
    if($pembimbing1)
    {
         $data['kd_dosen1']=$pembimbing1->pembimbing;
           $data['nm_dosen1']=$pembimbing1->nm_dosen;
            $data['jafung1']=$pembimbing1->jafung;
    }else{
         $data['kd_dosen1']='';
           $data['nm_dosen1']='';
            $data['jafung1']='';
     ...
     if($pembimbing2)
     {
          $data['kd_dosen2']=$pembimbing2->pembimbing;
           $data['nm_dosen2']=$pembimbing2->nm_dosen;
            $data['jafung2']=$pembimbing2->jafung;
     }else{
         $data['kd_dosen2']='';
           $data['nm_dosen2']='';
            $data['jafung2']='';
     ...
     $data['nim']=$usulan->nim;
      $data['nm_mahasiswa']=$usulan->nm_mahasiswa;
      $data['angkatan']=$usulan->angkatan;
      
     $data['no_daftar']=$no_daftar;
      $data['tgl_daftar']=$usulan->tgl_daftar;
    $keydosen['kd_fak']=$kd_fak;
    
    $data['dosen']=$this->Prodi_model->get_list_selected('vdosen',$keydosen);
...
function nilai_kelas($kd_jadwal) 
    {
        ...
        if (empty($cek)) {
            ...
            if ($level == "prodi") {
                ...
                $hasil3=$this->get_jadwal_detail($kd_jadwal);
        if($hasil3)
        {
...
public function reset_krs($no_krs)
{
   ...
   redirect(base_url().'prodi/allkrs');
...
function rekap_sks_lulus()
{
    ...
    $list_mhs=$this->Akademika_model->get_list_selected('vmahasiswa',$key);
...
function beasiswa()
{
     $this->cek();
       ...
       $data['list']=$a;
       foreach($a as $row){
           $xdata['beasiswa']=$row->jenis_beasiswa;
           $xkey['nim']=$row->nim;
           $this->Akademika_model->update_data('mahasiswa',$xdata,$xkey);
       }
...
function aakm()
{
    ...
    function edetail_mtk($kd_mtk,$ke)
    {
         $this->cek();
        $key['kd_mtk']=$kd_mtk;
        $key['pertemuan']=$ke;
        $matakuliahd=$this->Akademika_model->get_row_selected('matakuliahd',$key);
         $data['kd_mtk']=$kd_mtk;
         $data['pertemuan']=$ke;
         $data['sub_cp_mk']=$matakuliahd->sub_cp_mk;
         $data['indikator']=$matakuliahd->indikator;
         $data['kriteria_bentuk_penilaian']=$matakuliahd->kriteria_bentuk_penilaian;
         $data['metode_pembelajaran']=$matakuliahd->metode_pembelajaran;
         $data['materi_pembelajaran']=$matakuliahd->materi_pembelajaran;
         $data['bobot_penilaian']=$matakuliahd->bobot_penilaian;
    ...
    function adetail_matakuliah()
    {
        ...
        $pertemuan=$this->input->post('pertemuan');
         ...
         $cek_row=$this->Akademika_model->get_row_selected('matakuliahd',$key);
         if($cek_row)
         {
             $this->Akademika_model->update_data('matakuliahd',$data,$key);
         }else
         {
              $data['kd_mtk']=$kd_mtk;
        $data['pertemuan']=$pertemuan;
             $this->Akademika_model->save_data('matakuliahd',$data);
         }
       

            $this->session->set_flashdata('msg', '<div class="alert alert-info">
                    <h4>Informasi...!</h4><p>Data detail rencana pembelajaran sukses tersimpan</p>
					</div>');
   ...
   private function jumlah_registrasi_angkatan_prodi($prodi, $tahun_ajaran) {
    ...
    function monitoring_kam()
    {
        ...
        if($level=="prodi")
        {
            ...
            $mhss=$this->Akademika_model->get_list_selected('vmahasiswa_pa',$keymhs);
            ...
            foreach($mhss as $mhs)
            {
                 ...
                 $krs=$this->Akademika_model->get_row_selected('rencanastudih',$keyregis);
                 if($krs){
                     $krsh='Ya';
                 }else{
                     $krsh='Tidak';
                 ...
                 $mahasiswa['status']=$mhs->status;
                  $mahasiswa['nm_dosen']=$mhs->nm_dosen;
   ...
   // $data['jadwal']=$this->Prodi_model->get_row_selected('jadwal',$key);
    ...
    function rekap_absensi()
    {
       ...
       // $kriteria['kd_prodi']=$homebase;
        ...
        $filter['kd_prodi']=$kd_prodi;
        
        if ($level == "prodi") {
           
            $datax['data'] = $this->Akademika_model->get_list_selected('vrekap_jumlah_absensi',$filter);
           
            $this->template->load($this->view, 'prodi/absen/rekap_absensi', $datax);
...
WHERE
  `jadwal`.`kd_jadwal` = '" . $kd_jadwal . "'";
    ...
    {
        $sql="SELECT rencanastudih.nim,mahasiswa.nm_mahasiswa,mahasiswa.angkatan FROM rencanastudih,rencanastudid,mahasiswa WHERE rencanastudih.no_krs=rencanastudid.no_krs AND rencanastudih.nim=mahasiswa.nim and rencanastudid.kd_jadwal= '".$kd_jadwal."' and rencanastudih.setujui_pa='Ya'";
          $hasil = $this->db->query($sql)->result();
        return $hasil;
    }
    function get_absensi2($kd_jadwal) {
        $sql = "SELECT
  ...
  `matakuliah`.`kd_jenis_mtk`, `mahasiswa`.`angkatan`, `jadwal`.`kd_dosen`, `matakuliah`.`sks`
FROM
  ...
  `matakuliah` ON `matakuliah`.`kd_mtk` = `jadwal`.`kd_mtk` 
WHERE
  `jadwal`.`kd_jadwal` = '" . $kd_jadwal . "' and rencanastudih.setujui_pa='Ya' ";
    ...
    public function rekap_nilai($id)
    {
        $u=0;
        $s=0;
        $key['kd_tahun_ajaran']=$id;
        $key['kd_prodi']='022';
        
        $data=$this->Akademika_model->get_list_selected('vnilai',$key);
        foreach($data as $row)
        {
            $datax['kd_tahun_ajaran']=$row->kd_tahun_ajaran;
            $datax['nim']=$row->nim;
            $datax['kd_mtk']=$row->kd_mtk_mtk;
            $datax['sks']=$row->sks_krs;
            $datax['nilai_angka']=$row->nilai_angka;
            $datax['nilai_huruf']=$row->nilai_huruf;
            $datax['tanggal']=date('Y-m-d');
            $keys['kd_tahun_ajaran']=$row->kd_tahun_ajaran;
            $keys['nim']=$row->nim;
            $keys['kd_mtk']=$row->kd_mtk_mtk;
            
            $ada=$this->Akademika_model->get_row_selected('nilai',$keys);
            if($ada)
            {
               $this->Akademika_model->update_data('nilai',$datax,$keys);
              $u++;
            }else{
                $this->Akademika_model->save_data('nilai',$datax);
                 $s++;
            }
           
            
        }
         echo 'save:'.$s.', update:'.$u;
    }
    
    
    public function get_mahasiswa($kd_prodi,$angkatan)
    {
         $sql = "select beasiswa,mahasiswa.nim as nim,nm_mahasiswa,NIK,angkatan,concat('62',RIGHT(no_hp,length(no_hp)-1)) as no_hp,nilai_ukt,semester,mahasiswa.status,kd_prodi from mahasiswa where  kd_prodi='". $kd_prodi . "' and angkatan='".$angkatan."'";
     ...
     function fsinkron_pembayaran()
    {
        ...
        $this->template->load($this->view,'prodi/ukt/fsinkron_keuangan',$data);
    }
    public function sinkron_pembayaran()
    {
        ...
        $kd_prodi_sidu=$prodi->kd_sidu;
        //echo $kd_prodi_sidu;
        $url="https://sidu.usn.ac.id/web/sidu_siakad_ukt_prodi.php?ta='$ta'&prodi='$kd_prodi_sidu'";
        $result=file_get_contents($url);
     ...
     $jumlah=0;
       foreach ($hasil as $row)
       {
       	...
       	$this->session->set_flashdata('msg', '<div class="alert alert-warning">
                    <h4>Pesan</h4>
                    <p>Update data UKT '.$jumlah.' Record...!!!</p>
					</div>');
      $this->lreg($ta);
    }
    ...
    function amatakuliah() {
        ...
        if (empty($cek)) {
            ...
            if ($level == "prodi") {
               ...
               {
                   $id_jns_mtk='C';
               }elseif(substr($kd_jenis_mtk,0,3)=='MKW')
               {
                    $id_jns_mtk='A';
...
//FORM PEMBIMBING AKADEMIK
    ...
    function dpa($no_pa) {
        ...
        if (empty($cek)) {
            ...
            $id['no_pa'] = $no_pa;
            $this->Prodi_model->delete_data('pad', $id);
           $this->Prodi_model->delete_data('pah', $id);
    ...
    function get_registrasi($kd_tahun_ajaran,$homebase,$angkatan,$jenis_registrasi,$status)
    {
        if($status=='Sudah')
        {
            $sql="SELECT mahasiswa.beasiswa,registrasi.nim,noreg,tgl_reg_bak,tgl_reg_bank,nm_mahasiswa,angkatan,nilai_ukt,status FROM `registrasi`,mahasiswa WHERE registrasi.nim=mahasiswa.nim and kd_tahun_ajaran='".$kd_tahun_ajaran."' and registrasi.home_base='".$homebase."' and mahasiswa.angkatan='".$angkatan."' and jns_registrasi='P03' order by nim asc";    
        }else
        {
            $sql="select mahasiswa.beasiswa, mahasiswa.nim, nm_mahasiswa,angkatan,nilai_ukt,mahasiswa.status from mahasiswa where  status!='L' and nim not in (select nim from registrasi where kd_tahun_ajaran='".$kd_tahun_ajaran."') and angkatan='".$angkatan."' and kd_prodi='".$homebase."' order by nm_mahasiswa asc";
        }
    ...
    function ajm() {
        ...
        if (empty($cek)) {
            ...
            if ($level == "prodi") {
                ...
                $datax['link_virtual'] = $this->input->post('link_virtual', TRUE);
       ...
       $this->cek_jadwal($kd_jadwali);
        ...
        if (empty($cek)) {
            ...
            if ($level == "prodi") {
                ...
                $keymtks['kd_mtk']=$hasil->kd_mtk;
              $mtk=$this->Prodi_model->get_row_selected('matakuliah', $keymtks);
                //$datax['kd_dosen'] = $hasil->kd_dosen;
               // $datax['kd_dosen2'] = $hasil->kd_dosen2;
                $datax['nm_mtk'] = $mtk->nm_mtk;
                ...
                $datax['link_virtual'] = $hasil->link_virtual;
                $this->template->load($this->view, 'prodi/jadwalmatakuliah', $datax);
            } else {
                $this->session->sess_destroy();
                redirect(base_url());
...
$data['group_wa']=$row->group_wa;
    ...
    function transkrip_nilai2($nim)
    {
        ...
        $keynilai['nim']=$nim;
         $data['transkrip'] = $this->Akademika_model->get_list_selected('vnilai_transkrip',$keynilai);
...
GROUP BY  `rencanastudih`.`nim`, `rencanastudih`.`no_krs`";
    ...
    function tidak_aktif()
    {
         ...
         $filter['nim']=$this->input->post('nim');
         $datax['status']='T';
           $cek= $this->Akademika_model->update_data('mahasiswa',$datax,$filter);
           if($cek)
           {
                 echo 'Berhasil';
           }
    ...
    function save_mhs_keluar($nim)
    {
        $filter['nim']=$nim;
        $ada=$this->Akademika_model->get_row_selected('mahasiswa_keluar',$filter);
        $data['nim']=$nim;
         $data['kd_tahun_ajaran']=$this->session->userdata('kd_tahun_ajaran');
         $data['tgl']=date('Y-m-d');
        if(empty($ada))
        {
            $this->Akademika_model->save_data('mahasiswa_keluar',$data);
            $datax['status']='K';
            $this->Akademika_model->update_data('mahasiswa',$datax,$filter);
        }
        $this->get_rekap_krs_mhs2();
    ...
    function get_usulan_do2()
    {
        ...
        $kd_ta_awal=$this->session->userdata('kd_ta_awal');
        $kd_ta_akhir=$this->session->userdata('kd_ta_akhir');
        $data['ta_awal']=$kd_ta_akhir;
        $data['ta_akhir']=$kd_ta_akhir;
        $list=array();
        $sql = "select mahasiswa.beasiswa,nm_prodi,mahasiswa.nim as nim,nm_mahasiswa,angkatan,concat('62',RIGHT(no_hp,length(no_hp)-1)) as no_hp,nilai_ukt,semester,mahasiswa.status,awal_semester from mahasiswa,prodi where  mahasiswa.kd_prodi=prodi.kd_prodi and mahasiswa.kd_prodi='" . $kd_prodi . "' and angkatan='".$angkatan."' and mahasiswa.status='A' order by nm_mahasiswa asc";
        $listmhs= $this->db->query($sql)->result();
        foreach($listmhs as $mhs)
        {
            $sql2="SELECT * FROM `thnajaran` where `kd_tahun_ajaran` >='".$kd_ta_awal."' and kd_tahun_ajaran<='".$kd_ta_akhir."' order by kd_tahun_ajaran asc";   
            $listta= $this->db->query($sql2)->result();
            $data['listta']=$listta;
            //echo json_encode($listta);
            foreach($listta as $ta)
            ...
            $rec_ipk=$this->Akademika_model->get_ipk_mhs($mhs->nim,$kd_ta_akhir);
            $rec_jum_sks=$this->Akademika_model->get_jum_sks_mhs($mhs->nim,$kd_ta_akhir);
           // $rec_ips=$this->get_ips($mhs->nim,$kd_tahun_ajaran);
            $data['nim']=$mhs->nim;
            $data['nm_mahasiswa']=$mhs->nm_mahasiswa;
              $data['beasiswa']=$mhs->beasiswa;
            $data['angkatan']=$mhs->angkatan;
          ...
          $data['list']=$list;
              ...
              //$data['list']=$this->get_rekap_krs_mhs2();
            //  $this->load->view('prodi/lap_rekap_krs_mhs', $data);
              $this->template->load($this->view, 'prodi/do/fudo', $data);
              // $this->template->load($this->view, 'prodi/lap_rekap_krs_mhs_form', $data);
    }
    function get_usulan_do()
    {
        $angkatan=$this->input->post('angkatan',TRUE);
        $kd_ta_awal=$this->input->post('kd_tahun_ajaran',TRUE);
        $kd_ta_akhir=$this->input->post('kd_tahun_ajaran2',TRUE);
        
          $sess_data['kd_ta_awal']=$kd_ta_awal;
         $sess_data['kd_ta_akhir']=$kd_ta_akhir;
          $sess_data['angkatan']=$angkatan;
            $this->session->set_userdata($sess_data);
       $this->get_usulan_do2();
    }
    function get_rekap_krs_mhs()
    {
    ...
    public function get_rekap_krs_mhs2()
    {
        $kd_prodi=$this->session->userdata('home_base');
         $angkatan=$this->session->userdata('angkatan');

        $kd_tahun_ajaran=$this->session->userdata('kd_ta_awal');
        $kd_tahun_ajaran2=$this->session->userdata('kd_ta_akhir');
        $data['ta_awal']=$kd_tahun_ajaran;
        $data['ta_akhir']=$kd_tahun_ajaran2;
        
        $list=array();
        $sql = "select mahasiswa.beasiswa,nm_prodi,mahasiswa.nim as nim,nm_mahasiswa,angkatan,concat('62',RIGHT(no_hp,length(no_hp)-1)) as no_hp,nilai_ukt,semester,mahasiswa.status,awal_semester from mahasiswa,prodi where  mahasiswa.kd_prodi=prodi.kd_prodi and mahasiswa.kd_prodi='" . $kd_prodi . "' and angkatan='".$angkatan."' order by nm_mahasiswa asc";
        $listmhs= $this->db->query($sql)->result();
        foreach($listmhs as $mhs)
        {
    ...
    public function get_rekap_usul_do()
    {
        $kd_prodi=$this->session->userdata('home_base');
         $angkatan=$this->session->userdata('angkatan');
...
function berita_acara_ujian($no_daftar)
{
    ...
    $home_base=$this->session->userdata('home_base');
    $sql="SELECT nm_dosen,pembimbing_ke,jafung,jenis_ujian FROM `daftar`,daftar_judul,pembimbing_skripsi,dosen,jenis_ujian_prodi WHERE daftar.no_daftar_judul=daftar_judul.no_daftar AND daftar_judul.no_daftar=pembimbing_skripsi.no_daftar and pembimbing_skripsi.pembimbing=dosen.kd_dosen and daftar.urutan=jenis_ujian_prodi.urutan and jenis_ujian_prodi.kd_prodi=daftar.kd_prodi and daftar.no_daftar='".$no_daftar."'  order by pembimbing_ke asc ";
      $sql2="select daftar.nim,nm_mahasiswa,judul, kd_tahun_ajaran,no_daftar,tgl_daftar,tgl_ujian,jam,urutan from daftar,mahasiswa where daftar.nim=mahasiswa.nim and no_daftar='".$no_daftar."' ";
...
function delete_daftar_ujian($no_daftar)
{
    ...
    $data['jam']='';
     $data['ruang']='';
    $this->Prodi_model->update_data('daftar',$data,$key);
    $this->Prodi_model->delete_data('penguji',$key);
    redirect(base_url() . 'prodi/ujian_terjadwal');
...
function ujian_terjadwal()
{
    $kd_prodi=$this->session->userdata('home_base');
    $sql="SELECT daftar.no_daftar,tgl_daftar,tgl_ujian,jam,no_daftar_judul,nm_mahasiswa,concat('62',RIGHT(no_hp,length(no_hp)-1)) as no_hp,daftar.nim,judul,jenis_ujian,daftar.status from daftar,mahasiswa,jenis_ujian_prodi where daftar.nim=mahasiswa.nim and daftar.urutan=jenis_ujian_prodi.urutan and mahasiswa.kd_prodi=jenis_ujian_prodi.kd_prodi and  daftar.status='1' and daftar.kd_prodi='".$kd_prodi."' order by tgl_ujian,jam asc";
...
function pendaftar_ujian()
{
    $kd_prodi=$this->session->userdata('home_base');
    $sql="SELECT daftar.no_daftar,tgl_daftar,tgl_ujian,jam,no_daftar_judul,nm_mahasiswa,concat('62',RIGHT(no_hp,length(no_hp)-1)) as no_hp,daftar.nim,judul,jenis_ujian,daftar.status from daftar,mahasiswa,jenis_ujian_prodi where daftar.nim=mahasiswa.nim and daftar.urutan=jenis_ujian_prodi.urutan and mahasiswa.kd_prodi=jenis_ujian_prodi.kd_prodi and  daftar.status='0' and daftar.kd_prodi='".$kd_prodi."'  order by tgl_ujian,jam asc";
      
     $data['list'] = $this->db->query($sql)->result();
     $data['list2'] = $this->Prodi_model->get_all('status_ujian');
     $this->template->load($this->view, 'prodi/list_pendaftar_ujian', $data);
}
...
function list_daftar_judul()
{
    ...
    foreach($list as $row)
    {
        $pembimbing=$this->get_pembimbing_skripsi($row->no_daftar);
        $data['no_daftar']=$row->no_daftar;
        $data['tgl_daftar']=$row->tgl_daftar;
        $data['kd_tahun_ajaran']=$row->kd_tahun_ajaran;
        $data['nim']=$row->nim;
        $data['nm_mahasiswa']=$row->nm_mahasiswa;
         $data['angkatan']=$row->angkatan;
...
function get_pembimbing_skripsi($no_daftar)
{
      $sql="SELECT no_daftar,pembimbing_ke,pembimbing,nm_dosen,nidn,jafung FROM `pembimbing_skripsi`,dosen where pembimbing_skripsi.pembimbing=dosen.kd_dosen and no_daftar='".$no_daftar."'";
    $list='';
    $hasil= $this->db->query($sql)->result();
    if($hasil)
    {
        foreach($hasil as $row)
        {
            $list=$list.$row->pembimbing_ke.'). '.$row->nm_dosen.'<br>';
           
        }
    }
    return $list;
}
function usulan_judul_lulus($no_daftar){
    $filter['no_daftar']=$no_daftar;
    $data['status']='Skripsi';
    $this->Prodi_model->update_data('daftar_judul',$data,$filter);
      redirect(base_url() . 'prodi/list_daftar_judul');
}
    ...
    }else
    {
        ...
        $this->Prodi_model->update_data('daftar_judul',$hdata,$keyhdata);
            ...
            $data1['pembimbing_ke']=1;
             ...
             $data2['pembimbing_ke']=2;
             $data2['pembimbing']=$this->input->post('kd_dosen2',true);
             $this->Prodi_model->replace_data('pembimbing_skripsi',$data1,$keydata1);
              $this->Prodi_model->replace_data('pembimbing_skripsi',$data2,$keydata2);
...
//echo $inisial.$home_base.$kd_tahun_ajaran.$kd;
	...
	{
	     $sql="select daftar.no_daftar_judul,daftar.nim,no_daftar,tgl_daftar,tgl_ujian,jam,ruang,judul,nm_mahasiswa,angkatan,daftar.urutan,jenis_ujian from daftar,mahasiswa,jenis_ujian_prodi where daftar.nim=mahasiswa.nim and daftar.kd_prodi=jenis_ujian_prodi.kd_prodi and daftar.urutan=jenis_ujian_prodi.urutan and daftar.no_daftar='".$no_daftar."'";
	...
	function unilai_ujian()
	{
        ...
        if($lulus=='Y')
        {
            if($ujian->urutan=='1' and $lulus=='Y')
            {
                $udata['status']='Hasil';
            }elseif($ujian->urutan=='2' and $lulus=='Y')
            {
                 $udata['status']='Skripsi';
            }elseif($ujian->urutan=='0' and $lulus=='Y')
            {
...
function fedaftar_ujian($no_daftar)
{
    ...
    $data['dosen']=$this->Prodi_model->get_list_selected('vdosen',$keyfak);
...
function fnilai_ujianx($no_daftar)
{
    $homebase=$this->session->userdata('home_base');
    $keyprodi['kd_prodi']=$homebase;
    $data['aksi']="edit";
   ...
   else
   {
    ...
    $data['aksi']='edit';
     $data['listjenis_ujian']=$this->Prodi_model->get_list_selected('jenis_ujian_prodi',$keyprodi);
     $this->template->load($this->view, 'prodi/fnilai_ujian', $data);
...
function reset_password($userid)
    {
			...
			if($hasil2)
			{
					$this->session->set_flashdata('msg', '<div class="alert alert-info">
                    <h4>Pesan</h4>
                    <p>Password berhasil direset ke Password Awal: ('.$pass2.') ...!!!</p>
					</div>');
			}else
			{
			    	$this->session->set_flashdata('msg', '<div class="alert alert-info">
